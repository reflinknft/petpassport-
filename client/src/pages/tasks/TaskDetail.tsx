    ...
    <div className="min-h-full bg-brand-cream flex flex-col">
      ...
      <div className="flex-1 pb-28">
        ...
        <div className="relative h-44">
          ...
          <span className="absolute top-4 left-5 text-[11px] font-bold px-2.5 py-1 rounded-full bg-white/90 text-brand-brown flex items-center gap-1">
        ...
        <div className="px-5 -mt-4 relative">
          <div className="paper-card p-5 journal-enter">
            <div className="flex items-start justify-between gap-3">
              <h1 className="text-xl font-black text-brand-brown leading-snug">{task.title}</h1>
              <span className="shrink-0 text-brand-orange font-black text-lg tabular">+{task.points} 點</span>
            </div>
            <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{task.description}</p>
            <div className="mt-3 flex gap-2 text-[11px]">
              <span className="px-2.5 py-1 rounded-full bg-brand-apricot text-brand-brown font-bold">期限：{task.deadline}</span>
              {task.petCondition && <span className="px-2.5 py-1 rounded-full bg-brand-matcha/15 text-brand-matcha font-bold">{task.petCondition}</span>}
            </div>
            {task.status === "ongoing" && (
              <div className="mt-4">
                <div className="flex justify-between text-xs mb-1.5">
                  <span className="font-bold text-brand-brown">完成進度</span>
                  <span className="font-extrabold text-brand-orange tabular">{task.progress}%</span>
                </div>
                <div className="h-2 rounded-full bg-brand-apricot overflow-hidden">
          ...
          <div className="mt-4 paper-card p-5 journal-enter journal-enter-1">
            <h2 className="font-black text-brand-brown text-sm">完成步驟</h2>
            <ol className="mt-3 space-y-3">
              {task.steps.map((s, i) => (
                <li key={i} className="flex gap-3">
                  <span className="w-6 h-6 rounded-full bg-brand-apricot text-brand-brown text-xs font-black flex items-center justify-center shrink-0">{i + 1}</span>
          ...
          <div className="mt-4 paper-card p-5 journal-enter journal-enter-2">
            <h2 className="font-black text-brand-brown text-sm">驗證方式</h2>
            ...
            {task.type === "visit" && (
              <button className="mt-3 w-full h-10 rounded-xl border-2 border-dashed border-brand-orange/40 text-brand-orange text-sm font-bold flex items-center justify-center gap-2 active:scale-[0.97] transition-transform">
      ...
      <div className="sticky bottom-0 bg-brand-cream/95 backdrop-blur border-t border-border/60 p-4">
        <button
          ...
          className="w-full h-12 rounded-2xl bg-brand-orange text-white font-bold shadow-lg shadow-brand-orange/30 active:scale-[0.97] transition-all disabled:opacity-50 disabled:shadow-none flex items-center justify-center gap-2"
        >
          {ctaMap[status]}
      ...
      {/* 離站提醒 Sheet */}
      {showLeaveTip && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-brand-brown/40" onClick={() => setShowLeaveTip(false)}>
          <div className="w-full max-w-[390px] bg-white rounded-t-3xl p-6 pb-8 journal-enter" onClick={(e) => e.stopPropagation()}>
            <div className="w-10 h-1 rounded-full bg-border mx-auto mb-5" />
            <h2 className="font-black text-brand-brown">即將前往{task.type === "content" ? " HEHO Pet" : " Teachify"}</h2>
            ...
            <button
              ...
              className="mt-6 w-full h-12 rounded-2xl bg-brand-orange text-white font-bold active:scale-[0.97] transition-transform"
            >
