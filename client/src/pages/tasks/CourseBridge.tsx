      ...
      <div className="flex-1 px-5 pt-5 pb-8 flex flex-col">
        ...
        <div className="mt-4 paper-card p-5 journal-enter journal-enter-1">
          ...
          <ol className="mt-3 space-y-2.5 text-xs text-brand-sub">
            <li className="flex gap-2"><span className="w-5 h-5 rounded-full bg-brand-lilac text-brand-purple font-bold flex items-center justify-center shrink-0">1</span>前往 Teachify 課程頁（使用專屬連結識別來源）</li>
        ...
        <div className="mt-4 paper-card p-5 journal-enter journal-enter-2">
          ...
          <p className="mt-2 text-xs text-brand-sub leading-relaxed">
            DEMO 階段採「外部連結＋模擬完成」；MVP 階段將改為「優惠碼＋回站問答」，未來可透過 Teachify 匯出名單或 Webhook 自動回傳。
          </p>
        </div>

        <div className="mt-auto pt-6 space-y-3">
          <button
            onClick={() => window.open("https://www.teachify.com.tw", "_blank")}
          ...
          >
            <ExternalLink size={17} /> 前往 Teachify 上課
          </button>
          <button
