  ...
  return (
    <div className="min-h-full bg-brand-cream">
      ...
      <div className="px-5 pt-5 pb-8">
        {/* 主卡 */}
        <div className="rounded-3xl bg-brand-brown text-white p-6 shadow-xl shadow-brand-brown/25 rotate-[-0.5deg] journal-enter">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            ...
            <button onClick={() => navigate("/rewards")} className="rounded-2xl bg-brand-honey/20 p-3 text-left active:scale-95 transition-transform">
              <p className="text-[10px] text-brand-honey flex items-center gap-1"><Flame size={10} /> 即將到期</p>
              <p className="mt-1 font-extrabold text-brand-honey tabular">{MEMBER.expiringPoints} 點</p>
            </button>
          </div>
          <div className="mt-4 flex gap-3">
            <button
              onClick={() => navigate("/tasks")}
              className="flex-1 h-11 rounded-xl bg-brand-orange font-bold text-sm active:scale-[0.97] transition-transform"
            >
              去賺點
            </button>
            <button
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 journal-enter journal-enter-1">
          <div className="paper-card p-4">
            <p className="text-[11px] text-muted-foreground flex items-center gap-1"><ArrowUpRight size={12} className="text-brand-matcha" /> 本月取得</p>
            <p className="mt-1 text-xl font-black text-brand-matcha tabular">+{MEMBER.monthEarned}</p>
          </div>
          <div className="paper-card p-4">
        ...
        <div className="mt-6 flex items-center justify-between">
          <h2 className="font-black text-brand-brown">最近明細</h2>
          <button onClick={() => navigate("/wallet/history")} className="text-xs font-bold text-brand-orange flex items-center">
        ...
        <div className="mt-3 paper-card divide-y divide-border/70 journal-enter journal-enter-2">
          {recent.map((tx) => (
            <button
              ...
              className="w-full flex items-center gap-3 p-4 text-left active:bg-brand-apricot/40 transition-colors"
            >
              ...
              <span className={`font-extrabold text-sm tabular shrink-0 ${tx.points > 0 ? "text-brand-matcha" : "text-brand-brick"}`}>
        ...
        <button className="mt-4 w-full passport-frame p-4 flex items-center gap-3 bg-white/60 active:scale-[0.98] transition-transform journal-enter journal-enter-3">
          ...
          <p className="text-xs text-muted-foreground text-left">
            點數如何取得、使用與到期？<span className="font-bold text-brand-brown">查看點數規則</span>
          </p>
        </button>
      </div>
    </div>
