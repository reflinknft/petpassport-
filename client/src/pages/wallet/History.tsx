...
export default function History() {
  ...
  return (
    <div className="min-h-full bg-brand-cream">
        ...
        {list.map((tx, i) => (
          ...
          >
            ...
            <div className="text-right shrink-0">
              <span className={`font-extrabold text-sm tabular ${tx.points > 0 ? "text-brand-mint" : "text-brand-brick"}`}>
                {tx.points > 0 ? `+${tx.points}` : tx.points}
              </span>
              {tx.status === "pending" && <p className="text-[10px] font-bold text-brand-coral">待生效</p>}
            </div>
          </button>
        ))}
        {list.length === 0 && (
      ...
      {/* 交易詳情 Bottom Sheet */}
      {sel && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-brand-purple-dark/40" onClick={() => setSelected(null)}>
          <div className="w-full max-w-[390px] bg-white rounded-t-3xl p-6 pb-8 journal-enter" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 className="mt-1 font-black text-brand-ink">{sel.title}</h2>
            <p className={`mt-3 text-3xl font-black tabular ${sel.points > 0 ? "text-brand-mint" : "text-brand-brick"}`}>
              {sel.points > 0 ? `+${sel.points}` : sel.points} 點
            </p>
            <div className="mt-5 space-y-2.5 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">日期</span><span className="font-bold text-brand-ink">{sel.date}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">來源</span><span className="font-bold text-brand-ink">{sel.source}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">稽核編號</span><span className="font-mono text-xs font-bold text-brand-ink">{sel.serial}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">狀態</span><span className="font-bold text-brand-ink">{sel.status === "pending" ? "待生效" : "已完成"}</span></div>
            </div>
            <button className="mt-6 w-full h-11 rounded-xl border-2 border-brand-purple-dark/15 text-brand-ink text-sm font-bold flex items-center justify-center gap-2 active:scale-[0.97] transition-transform">
