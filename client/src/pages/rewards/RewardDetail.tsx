        ...
        <div className="px-5 -mt-6 relative">
          <div className="paper-card p-5 journal-enter">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h1 className="text-xl font-black text-brand-brown">{reward.title}</h1>
                <p className="mt-1 text-xs text-muted-foreground">{reward.refValue}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="text-2xl font-black text-brand-orange tabular">{reward.points}</p>
            ...
            <div className="mt-3 flex gap-2 text-[11px] flex-wrap">
              <span className="px-2.5 py-1 rounded-full bg-brand-apricot text-brand-brown font-bold">{reward.category}</span>
              <span className="px-2.5 py-1 rounded-full bg-brand-apricot text-brand-brown font-bold">兌換後 {reward.validDays} 天內有效</span>
              <span className={`px-2.5 py-1 rounded-full font-bold ${reward.stock < 20 ? "bg-brand-brick/10 text-brand-brick" : "bg-brand-matcha/15 text-brand-matcha"}`}>
                剩餘 {reward.stock} 份
              </span>
            </div>
          </div>
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-brand-apricot flex items-center justify-center shrink-0">
              <Store size={18} className="text-brand-brown" />
            </div>
            <div className="text-left">
              <p className="text-sm font-bold text-brand-brown">{reward.merchant}</p>
          ...
          <div className="mt-4 paper-card p-5 journal-enter journal-enter-2">
            <h2 className="font-black text-brand-brown text-sm">兌換與使用規則</h2>
            <ul className="mt-3 space-y-2">
              {reward.terms.map((t, i) => (
                <li key={i} className="flex gap-2 text-xs text-muted-foreground leading-relaxed">
                  <span className="text-brand-orange font-black">·</span> {t}
                </li>
              ))}
              <li className="flex gap-2 text-xs text-muted-foreground leading-relaxed">
                <span className="text-brand-orange font-black">·</span> 兌換後恕不退換點數；逾期自動失效。
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="sticky bottom-0 bg-brand-cream/95 backdrop-blur border-t border-border/60 p-4">
        <div className="flex items-center gap-3">
          <div className="shrink-0">
            ...
            <p className={`font-black tabular ${enough ? "text-brand-brown" : "text-brand-brick"}`}>{points.toLocaleString()}</p>
          </div>
          {enough ? (
            <button
            ...
            <button
              ...
              className="flex-1 h-12 rounded-2xl bg-brand-brown text-white font-bold active:scale-[0.97] transition-transform"
            >
              點數不足，去賺點
            </button>
          )}
