    ...
    <div className="min-h-full bg-brand-cream flex flex-col">
      ...
      <div className="flex-1 px-5 pt-5 pb-8">
        <div className="paper-card overflow-hidden journal-enter">
          ...
          <div className="p-5">
            <h1 className="text-lg font-black text-brand-brown">{reward.title}</h1>
          ...
          <div className="p-5 space-y-3">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-brand-brown tabular">{points.toLocaleString()} 點</span>
            </div>
            <div className="h-px bg-border" />
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">兌換後餘額</span>
              <span className="font-black text-brand-brown tabular">{(points - reward.points).toLocaleString()} 點</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">票券效期</span>
              <span className="font-bold text-brand-brown">兌換後 {reward.validDays} 天</span>
            </div>
          </div>
        </div>
        ...
        <label className="mt-5 flex items-start gap-2.5 cursor-pointer journal-enter journal-enter-1">
          ...
          <span className="text-xs text-muted-foreground leading-relaxed">
            我已了解兌換後<span className="font-bold text-brand-brown">恕不退換點數</span>，並同意本權益的使用規則。
          </span>
        </label>

        <div className="mt-4 passport-frame p-4 flex gap-3 bg-white/60 journal-enter journal-enter-2">
          <ShieldAlert size={16} className="text-brand-honey shrink-0 mt-0.5" />
        ...
        <button
          ...
          disabled={!agreed}
          className="mt-6 w-full h-12 rounded-2xl bg-brand-orange text-white font-bold shadow-lg shadow-brand-orange/30 active:scale-[0.97] transition-all disabled:opacity-40 disabled:shadow-none"
        >
